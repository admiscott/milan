...
server.use(session({
  secret:"128位字符串",//加密算法128位安全
  resave:true,//每次请求都更新并保存数据
...
server.get("/login",(req,res)=>{
   ...
   pool.query(sql,[uname,upass],(err,result)=>{
     ...
     if(result.length==0){
       ...
       req.session.uid=result[0].id;//req.session.id错误啊坑！session自己有id，=是赋值，不能改我啊
...
server.get("/product",(req,res)=>{
  ...
  sql+=" FROM xz_laptop";//前面要有个空格 分页查询重要啊！
  sql+=" LIMIT ?,?";
  var offset = (pno-1)*ps;
  ps = parseInt(ps);
  //3:json
